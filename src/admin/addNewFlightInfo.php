...
if (isset($_POST['insert'])) {
    ...
    try {
        $sql = "INSERT INTO flight (airline_id,flight_name,flight_date,destination,source,total_distance,fee_per_ticket,departure_time,arrival_time,capacity,gate,placeImg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $status = $stmt->execute([$airname, $fname, $date, $des, $ori, $tdistance, $price, $deptime, $arrtime, $cap, $gate, $uploadPath]);
