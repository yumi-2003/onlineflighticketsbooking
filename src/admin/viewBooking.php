<?php
    ...
    try{
      $sql = "SELECT booking.booking_id,users.username,flight.flight_name,classes.class_name,triptype.triptype_name,seat_layout.seatNo,booking.bookAt,booking.status,payment.totalPrice FROM booking 
      INNER JOIN users ON booking.user_id = users.user_id  
      INNER JOIN flight ON booking.flight_id = flight.flight_id 
      INNER JOIN classes ON booking.class_id = classes.class_id 
      INNER JOIN triptype ON booking.triptype_id = triptype.triptypeId
      INNER JOIN seat_layout ON booking.seatNOId = seat_layout.id 
      INNER JOIN payment ON booking.payment_id = payment.paymentID";
      $stmt = $conn->query($sql);
      $status = $stmt->execute();
         ...
         <div>
            ...
            <?php
               ...
               if(isset($_SESSION['updateFlightComplete'])){
                  echo "<div>
                           <span class='font-medium'>$_SESSION[updateFlightComplete]</span>
                        </div>
                        ";
