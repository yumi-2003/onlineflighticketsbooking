<?php
        ...
        if(isset($_POST['update'])){
            ...
            try{
                $sql = "Update airline set airline_name = ?, photo = ? where airline_id=?";

                $stmt = $conn->prepare($sql);
                $status = $stmt->execute([$airname,$uploadPath,$airline_Id]);
    ...
    <body>
        ...
        <form method="POST" action="<?php $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data" class="m-auto mt-16 w-60">
            <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-1">
                ...
                value = 
                "<?php
                if(isset($flight['airline_id'])){
                    echo $flight['airline_id'];
                }
                ?>">
                 <div>
